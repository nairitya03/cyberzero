...
class Knowledge(Tool):
    def execute(self, question:str = "", deep_search:str = "" , **kwargs):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            # # duckduckgo search
            duckduckgo = executor.submit(duckduckgo_search.deep_search, question, str(deep_search).lower().strip() == "true")
            # duckduckgo = executor.submit(web_search.AsyncDDGS)
            ...
            # clean_result=json.dump(duckduckgo_result)
        msg = files.read_file("prompts/tool.knowledge.response.md", 
                              online_sources = duckduckgo_result)
