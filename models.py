...
# Ollama models
def get_ollama_chat(model_name:str, temperature=DEFAULT_TEMPERATURE, base_url="https://034c-34-126-132-102.ngrok-free.app"):
...
# Groq models
def get_groq_chat(model_name:str = "llama-3.1-70b-versatile", api_key=None, temperature=DEFAULT_TEMPERATURE):
    api_key = api_key or get_api_key("groq")
    return ChatGroq(model_name=model_name, temperature=temperature, api_key=api_key)#, request_timeout = 30) # type: ignore

def get_mixtral_model (api_key=None, model:str = "mistral-large-latest", temperature:float = DEFAULT_TEMPERATURE):
    api_key = api_key or get_api_key("openai")
    return ChatOpenAI(openai_api_base="https://api.mistral.ai/v1", openai_api_key= api_key, model_name=model, temperature=temperature)# type: ignore

def get_mixtral_embeddingmodel (api_key=None, model:str = "mistral-large-latest"):
    api_key = api_key or get_api_key("openai")
    return OpenAIEmbeddings(openai_api_base="https://api.mistral.ai/v1", openai_api_key= api_key, model_name=model)# type: ignore

def get_openai_embedding(model_name:str, api_key=None):
    api_key = api_key or get_api_key("openai")
    return OpenAIEmbeddings(openai_api_base="https://api.mistral.ai/v1", model=model_name, openai_api_key=api_key) # type: ignore
